...
// SSBO for materials
layout(std430, binding = 1) buffer Materials { Material uMaterial[]; };

// SSBO for lights
layout(std430, binding = 2) buffer Lights { Light uLight[]; };
...
void main() {
  ...
  for (uint i = 0; i < nLights; i++) {
    ...
    case LIGHT_POINT: {
      attenuation = lightAttenuate(length(lightVec), uLight[i].range) *
                    uLight[i].intensity;
      if (uLight[i].shadowMapLayer >= 0) {
        // Shadow mapping for point light
        vec3 wLightVec = v_in.wposition - uLight[i].position;
        attenuation *=
            texture(uCubeShadowMaps, vec4(wLightVec, uLight[i].shadowMapLayer),
                    length(wLightVec) / uLight[i].far - 0.01);
      }
      break;
    }
    case LIGHT_SPOT: {
      float theta = dot(normalize(uLight[i].position - v_in.wposition),
      ...
      if (theta >= uLight[i].spotAngle) {
        ...
        if (uLight[i].shadowMapLayer >= 0) {
          ...
          attenuation *= texture(
              u2DShadowMaps,
              vec4(wLightVec.xy / (2.0 * wLightVec.w) + 0.5,
                   uLight[i].shadowMapLayer,
                   min(1.0, length(wLightVec.xyz / uLight[i].far) - 0.01)));
        }
      }
      break;
    }
    ...
    const vec3 specular =
        pow(max(0.0, dot(h, n)), max(0.1, mat.shininess)) * specularColor;

    C += attenuation * uLight[i].color *
         (diffuse + specular * mat.specularFactor);
  }

  // Environment mapping / reflection
  if (mat.reflectionLayer >= 0) {
    vec3 reflectionTex =
        texture(uReflectionMaps,
                vec4(reflect(v_in.wposition - uCamPos, normalize(v_in.wnormal)),
                     mat.reflectionLayer))
            .rgb;
    C = reflectionTex;
  }
  if (mat.flatReflectionTexID >= 0) {
    ...
    vec3 reflectionTex =
        texture(uTex[mat.flatReflectionTexID], vec2(ts.x, 1.0 - ts.y)).rgb;
    C = reflectionTex;
  }
